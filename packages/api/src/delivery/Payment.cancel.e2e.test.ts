...
describe('Pruebas E2E de Cancelación de Pagos', () => {
    ...
    it('Debe cancelar el pago en la base de datos real', async () => {
        const response = await app.inject({
            method: 'PATCH',
            url: `/api/v1/payments/${createdPaymentId}/cancel`,
        });

        expect(response.statusCode).toBe(200);
        const body = JSON.parse(response.payload);
        expect(body.data.status).toBe('Canceled');
        expect(body.data.paymentDate).toBeNull();
