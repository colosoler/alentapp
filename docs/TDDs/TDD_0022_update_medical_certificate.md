...
- Si la actualizacion es correcta, debe retornar los datos actualizados del certificado.
 - El sistema debe permitir actualizar o reemplazar el archivo PDF asociado al certificado.
 - Si se actualiza el archivo, debe validarse formato PDF y límite de 5MB.
...
Para la actualización del archivo, el endpoint aceptará `multipart/form-data` con el campo `file` (PDF) o bien un JSON parcial si sólo se actualizan campos textuales.
